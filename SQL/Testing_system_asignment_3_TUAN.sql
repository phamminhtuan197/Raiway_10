...
SELECT 
    DepartmentID, COUNT(Username) AS 'Số nhân viên'
FROM
    account
...
SELECT 
    d.DepartmentID,
    d.DepartmentName,
    COUNT(username) AS 'Số nhân viên'
FROM
    department d
        JOIN
...
FROM
    ...
    account a ON d.DepartmentID = a.DepartmentID
GROUP BY DepartmentID
HAVING DepartmentName IN ('Marketing', 'Sale')
ORDER BY DepartmentID;
...
SELECT 
    d.DepartmentID,
    d.DepartmentName,
    COUNT(PositionName) AS 'Số nhân viên Test',
    GROUP_CONCAT(FullName) AS 'Tên nhân viên Test'
FROM
    department d
        JOIN
    `account` a ON d.DepartmentID = a.DepartmentID
